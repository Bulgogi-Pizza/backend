...
import on.logistics.orderservice.domain.entity.QVendorOrder;
...
public class OrderQueryRepositoryImpl implements OrderQueryRepository {
  ...
  private BooleanBuilder getSearchOrderQuery(SearchOrderPageDto searchOrderPageDto) {
    ...
    if (searchOrderPageDto.ordererUserNickname() != null) {
      builder.and(
          order.orderer.userNickname.value.contains(searchOrderPageDto.ordererUserNickname()));
    }
  ...
  private OrderSpecifier<?>[] getOrderSpecifiers(Pageable pageable) {
    ...
    sort.forEach(orderBy -> {
      ...
      switch (PageSortBy.valueOf(sortBy.toUpperCase())) {
        case CREATED_AT -> orderSpecifiers.add(new OrderSpecifier<>(direction, order.createdAt));
        case UPDATED_AT -> orderSpecifiers.add(new OrderSpecifier<>(direction, order.updatedAt));
