...
import on.logistics.orderservice.application.clients.ai.AIServiceClient;
...
import on.logistics.orderservice.application.clients.company.CompanyServiceClient;
import on.logistics.orderservice.application.clients.company.feign.dtos.GetCompanyResponse;
import on.logistics.orderservice.application.clients.delivery.DeliveryServiceClient;
import on.logistics.orderservice.application.clients.delivery.dtos.DeliveryRequestDto;
import on.logistics.orderservice.application.clients.exception.ExternalApiException;
import on.logistics.orderservice.application.clients.product.ProductServiceClient;
...
public class OrderServiceImpl implements OrderService {
  ...
  private Orderer createOrderer(final Order createdOrder, final UUID ordererId) {
    ...
    GetCompanyResponse companyResponseDto = companyServiceClient.getCompanyById(ordererId);
  ...
  private GenerateShippingDeadlineResponse tryGenerateShippingDeadline(VendorOrder createdVendorOrder) {
    log.info("배송 예상일 생성 요청");
    var requestDto = GenerateShippingDeadlineRequestDto.from(createdVendorOrder);
    return aiServiceClient.generateShippingDeadline(requestDto);
  }
  ...
  private Vendor createVendor(
    ...
    var companyResponseDto = companyServiceClient.getCompanyById(ordersByVendor.vendorId());
  ...
  private OrderProduct createOrderProduct(
    ...
    var productResponseDto = productServiceClient.getProductById(orderedProduct.productId());
  ...
  private void decreaseProductStock(OrderedProduct orderedProduct) {
    ...
    productServiceClient.decreaseProductStock(requestDto);
  }
  ...
  private void rollbackDecreaseProductStock(OrderProduct orderProduct) {
    ...
    productServiceClient.rollbackDecreaseProductStock(requestDto);
  }

  private void deliveryRequest(VendorOrder vendorOrder) {
    log.info("배송 요청");
    var requestDto = DeliveryRequestDto.from(vendorOrder);
    deliveryServiceClient.deliveryRequest(requestDto);
  }
}
